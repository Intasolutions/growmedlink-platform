...
import { Plus, Edit, Trash2, AlertCircle, RefreshCw, Folder } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="border border-dashed border-white/10 rounded-2xl h-80 flex flex-col items-center justify-center bg-[#0A192F]/10 text-center p-6">
          ...
          <h3 className="text-white font-bold text-base mb-1">No categories registered</h3>
          <p className="text-gray-400 text-xs font-light max-w-sm mb-6">
            Get started by creating your first dynamic category.
          </p>
          <button
            onClick={openCreateModal}
                  ...
                  <tr key={cat._id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="py-4 px-6 text-right space-x-2 shrink-0">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg text-xs font-semibold border border-red-500/10 transition-all"
                      >
          ...
          <div className="bg-[#0A192F] border border-white/10 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div>
                ...
                />
                {formError.name && (
                  <p className="text-red-400 text-xs mt-1">{formError.name[0]}</p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {formError.slug && (
                  <p className="text-red-400 text-xs mt-1">{formError.slug[0]}</p>
                )}
              </div>
              ...
              <div className="flex gap-3 justify-end pt-4 border-t border-white/5">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2.5 bg-white/5 hover:bg-white/10 text-white text-xs font-semibold rounded-lg border border-white/10 transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2.5 bg-secondary hover:bg-secondary-dark text-[#020C1B] text-xs font-bold rounded-lg transition-colors disabled:opacity-50"
                >
