...
export default function EditServicePage({ params }: EditServiceProps) {
    ...
    const fetchService = async () => {
      try {
        ...
        if (res.status === 200 && data.success) {
          ...
          setKeywordsInput(service.keywords?.join(', ') || '');
          setCanonicalUrl(service.canonicalUrl || '');
          setOgImage(service.ogImage || '');
        } else {
          setErrorMsg(data.message || 'Failed to retrieve service parameters.');
        }
      } catch (err) {
        console.error('[EditService] Fetch error:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[EditService] Submit error:', err);
      ...
      <div className="flex items-center gap-4 bg-[#0A192F]/40 border border-white/5 p-6 rounded-2xl">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5 font-light">
            Update program fields or configure SEO properties.
          </p>
        </div>
      </div>
