...
export default function EditProductPage({ params }: EditProductProps) {
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (res.status === 200 && data.success) {
          ...
          setSelectedImage(product.image as IMedia);
          setSecondaryImage(product.secondaryImage ? (product.secondaryImage as IMedia) : null);
          setDetailsText(product.details?.text || '');
          setFees(product.fees);
          ...
          setErrorMsg(data.message || 'Failed to retrieve product parameters.');
        }
      } catch (err) {
  ...
  // Auto-generate slug from name (only if autoSlug is checked)
  useEffect(() => {
    if (autoSlug) {
      ...
      <div className="flex items-center gap-4 bg-[#0A192F]/40 border border-white/5 p-6 rounded-2xl">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5 font-light">
            Update product fields or configure SEO properties.
          </p>
        </div>
      </div>
