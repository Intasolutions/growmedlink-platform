...
import { Plus, Edit, Trash2, Star, StarOff, AlertCircle, RefreshCw, ChevronLeft, ChevronRight, FileText } from 'lucide-react';
...
export default function AdminBlogsPage() {
  ...
  const handleDelete = async (id: string, title: string) => {
    if (!confirm(`Are you sure you want to delete the article "${title}"? This is a soft-delete and can be restored.`)) {
      return;
    }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'published':
        return 'bg-green-500/10 text-green-400 border border-green-500/15';
      case 'archived':
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="border border-dashed border-white/10 rounded-2xl h-80 flex flex-col items-center justify-center bg-[#0A192F]/10 text-center p-6">
          ...
          <h3 className="text-white font-bold text-base mb-1">No articles composed</h3>
          <p className="text-gray-400 text-xs font-light max-w-sm mb-6">
            Get started by composing your first rich educational article or announcement post.
          </p>
          <Link
            href="/admin/blogs/new"
