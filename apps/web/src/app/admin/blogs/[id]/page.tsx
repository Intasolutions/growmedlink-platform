...
export default function EditBlogPage({ params }: EditBlogProps) {
  ...
  const [autoSlug, setAutoSlug] = useState(false); // default false on edit to avoid accidental renames
    ...
    const fetchBlog = async () => {
      try {
        ...
        if (res.status === 200 && data.success) {
          ...
          setContent(blog.content || { type: 'doc', content: [] });
          setSelectedImage(blog.image as IMedia);
        ...
        console.error('[EditBlog] Fetch error:', err);
        setErrorMsg('Could not connect to the API server to retrieve details.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // Auto-generate slug from title (only if autoSlug is checked)
  useEffect(() => {
    if (autoSlug) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[EditBlog] Submit error:', err);
  ...
  if (isLoading) {
    return (
      <div className="h-96 flex flex-col items-center justify-center gap-3 bg-[#0A192F]/10">
        <Loader2 className="h-10 w-10 text-secondary animate-spin" />
        <span className="text-xs text-gray-400 font-semibold uppercase tracking-widest">Loading article parameter details...</span>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-4 bg-[#0A192F]/40 border border-white/5 p-6 rounded-2xl">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5 font-light">
            Update content fields, workflow status, or configure metadata.
          </p>
        </div>
      </div>
